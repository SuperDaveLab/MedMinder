...
- Repo path on server: `/opt/nexpill`
- API port: `8787`
...
```bash
sudo mkdir -p /opt/nexpill
sudo chown "$USER":"$USER" /opt/nexpill
cd /opt/nexpill
...
```sql
CREATE DATABASE IF NOT EXISTS nexpill
  CHARACTER SET utf8mb4
  COLLATE utf8mb4_unicode_ci;

CREATE USER IF NOT EXISTS 'nexpill_app'@'127.0.0.1' IDENTIFIED BY 'REPLACE_ME_STRONG_PASSWORD';
GRANT ALL PRIVILEGES ON nexpill.* TO 'nexpill_app'@'127.0.0.1';
FLUSH PRIVILEGES;
```
...
Create `/etc/nexpill/api.env`:

```bash
sudo mkdir -p /etc/nexpill
sudo tee /etc/nexpill/api.env > /dev/null <<'EOF'
AUTH_API_PORT=8787
...
sudo chmod 600 /etc/nexpill/api.env
```
...
Create `/etc/systemd/system/nexpill-api.service`:
...
[Service]
Type=simple
WorkingDirectory=/opt/nexpill
EnvironmentFile=/etc/nexpill/api.env
...
sudo systemctl enable nexpill-api
sudo systemctl restart nexpill-api
sudo systemctl status nexpill-api --no-pager
```

Logs:

```bash
journalctl -u nexpill-api -f
```
...
git pull --ff-only
npm ci
set -a; source /etc/nexpill/api.env; set +a
npm run api:init-db
sudo systemctl restart nexpill-api
sudo systemctl status nexpill-api --no-pager
```
